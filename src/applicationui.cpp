        ...
        QObject(app)
{
	...
	settings.setValue("appName", "BarcodeScanner");
	settings.setValue("version", version);
	settings.setValue("appKey", "614bf149-5f54-4c41-8df0-4ec1ef94cea1"); // generated randomly online

    t2w = new Talk2WatchInterface(this);
    connect(t2w, SIGNAL(transmissionReady()), this, SLOT(onTransmissionReady()));

    udp = new UdpModule;
    udp->listenOnPort(9110);
...
void ApplicationUI::onSystemLanguageChanged()
{
    ...
    QString file_name = QString("BarcodeScanner_%1").arg(locale_string);
...
void ApplicationUI::authorizeAppWithT2w()
{
	if (t2w->isTalk2WatchProInstalled() || t2w->isTalk2WatchProServiceInstalled()) {
		QString t2wAuthUdpPort = "9110"; // Random port, use the same value as the port in udp->listenOnPort(xxxx);
...
void ApplicationUI::onUdpDataReceived(QString _data)
{
	...
	if(_data=="AUTH_SUCCESS") {
		...
		t2wActionTitle << "Toggle ON/OFF";  // << "And so on..."
		t2wActionCommand << "BARCODE_TOGGLE";
		t2wActionDescription << "Toggle ON/OFF Barcode Scanner";
	...
	if (_data=="BARCODE_TOGGLE") {
		if (m_camera->isVisible()) {
			m_label->setText("Please restart the camera in ScriptMode");
			m_camera->close();
			m_camera->setVisible(false);
		}
		else {
			m_label->setText("Searching for a barcode...");
...
void ApplicationUI::sendToWatch(QString data, bb::cascades::multimedia::BarcodeFormat *format)
{
	Q_UNUSED(format);
	t2w->sendSms("Your barcode returned", data);
}
